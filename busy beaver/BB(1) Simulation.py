...
def transition(state, symbol):
    # 상태 A에서 0을 만나면 1을 쓰고 오른쪽으로 이동하며 종료
...
class A0to1RH(Scene):
    def construct(self):
        ...
        # 타이틀 및 전이 함수 텍스트
        title = Tex(r"$\delta(A, 0) \rightarrow (1, R, \textbf{H})$", font_size=42)
        ...
        final_state_text = Text("State: H", font_size=28, color=WHITE).next_to(title, DR, buff=1)

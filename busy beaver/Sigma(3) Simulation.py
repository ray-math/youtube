...
# Updated transition function based on the new transition rules
def transition(state, symbol):
    if state == 'A' and symbol == '0':
        return ('1', 'R', 'B')  # 상태 A, 0일 때 1 쓰고 오른쪽 이동, 상태 B로 전환
    elif state == 'A' and symbol == '1':
        return ('1', 'R', 'H')  # 상태 A, 1일 때 1 쓰고 오른쪽 이동, 종료 (H)
    elif state == 'B' and symbol == '0':
        return ('0', 'R', 'C')  # 상태 B, 0일 때 0 쓰고 오른쪽 이동, 상태 C로 전환
    elif state == 'B' and symbol == '1':
        return ('1', 'R', 'B')  # 상태 B, 1일 때 1 쓰고 오른쪽 이동, 상태 B 유지
    elif state == 'C' and symbol == '0':
        return ('1', 'L', 'C')  # 상태 C, 0일 때 1 쓰고 왼쪽 이동, 상태 C 유지
    elif state == 'C' and symbol == '1':
        return ('1', 'L', 'A')  # 상태 C, 1일 때 1 쓰고 왼쪽 이동, 상태 A로 전환
...
class Sigma3Simulation(Scene):
    def construct(self):
        ...
        # Title
        title = Tex(r"$\Sigma(3) \text{ Simulation}$", font_size=48)
        ...
        table_data = [["0", "1RB", "0RC", "1LC"], ["1", "1RH", "1RB", "1LA"]]
